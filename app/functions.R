...
#' env_ts <- build_ts_env(df_env, ts_res = "year")
#' }
#'
...
#'
#' @export
build_ts_env <- function(df_env, ts_res) {
...
#' sp_ts <- build_ts_sp(df_sp, ts_res = "year")
#' }
#'
...
#'
#' @export
build_ts_sp <- function(df_sp, ts_res) {
...
#'   showModal(depthProfileModal(map_sp = NULL))
#' })
#' }
#'
...
#'
#' @export
depthProfileModal <- function(map_sp) {
  modalDialog(
...
#' time series using DuckDB date functions. Used internally by \code{\link{build_ts_env}}.
#'
#' @param ts_res Character string specifying temporal resolution: "year", "quarter",
#'   "month", "day", "year_quarter", "year_month", or "year_day"
#'
...
#' }
#'
#' @seealso \code{\link{build_ts_env}} for usage context
#'
...
#' using DuckDB date functions. Used internally by \code{\link{build_ts_sp}}.
#'
#' @param ts_res Character string specifying temporal resolution: "year", "quarter",
#'   "month", "day", "year_quarter", "year_month", or "year_day"
#'
...
#' }
#'
#' @seealso \code{\link{build_ts_sp}} for usage context
#'
#' @importFrom rlang expr
#' @importFrom dbplyr sql
#'
...
#' }
#'
#' @seealso \code{\link{map_env_hex}} for spatial aggregation
#' @seealso \code{\link{build_ts_env}} for temporal aggregation
#'
...
get_env <- function(env_var, qtr, date_range, min_depth, max_depth) {
  if (debug) cat("get_env: env_var =", env_var, ", qtr =", paste(qtr, collapse = ","),
  ...
  if (debug) {
    ...
    cat("get_env: returning lazy table with", n_rows, "rows\n")
  }
...
#' @seealso \code{\link{build_ts_sp}} for temporal aggregation
#'
...
get_sp <- function(sp_name, qtr, date_range) {
  if (debug) cat("get_sp: sp_name =", sp_name, ", qtr =", paste(qtr, collapse = ","),
                 ", date_range =", paste(date_range, collapse = " to "), "\n")
...
#' \dontrun{
#' df_env <- get_env("t_deg_c", qtr = 1:4, date_range = c("2000-01-01", "2020-12-31"), min_depth = 0, max_depth = 100)
#' env_hex <- map_env_hex(df_env, res_range = 3:5, env_stat = "mean")
...
#' }
#'
#' @seealso \code{\link{map_env_hex}} for data aggregation
...
map_env <- function(env_hex_list, env_scale_list, env_stat_label, env_var_label) {
  if (debug) {
    cat("map_env: creating environmental map with", length(env_hex_list), "resolution layers\n")
    cat("map_env: first layer has", nrow(env_hex_list[[1]]), "hexagons\n")
  }
...
#'
#' @export
map_env_hex <- function(df_env, res_range, env_stat) {
  if (debug) cat("map_env_hex: aggregating env data for resolutions", paste(res_range, collapse = ","),
                 ", stat =", env_stat, "\n")
  ...
  if (debug) cat("map_env_hex: collected", nrow(hex_env_collected), "hex records before join\n")
  ...
  if (debug) {
    cat("map_env_hex: created", length(hex_env), "hex layers\n")
    for (i in seq_along(hex_env)) {
      cat("  Resolution", res_range[i], ":", nrow(hex_env[[i]]), "hexagons\n")
    }
  }
...
#' sp_hex <- map_sp_hex(df_sp, res_range = 3:5)
...
map_sp <- function(sp_hex_list, sp_scale_list) {
  if (debug) {
    cat("map_sp: creating species map with", length(sp_hex_list), "resolution layers\n")
    cat("map_sp: first layer has", nrow(sp_hex_list[[1]]), "hexagons\n")
  }
...
#'
#' @export
map_sp_hex <- function(df_sp, res_range) {
  if (debug) cat("map_sp_hex: aggregating species data for resolutions", paste(res_range, collapse = ","), "\n")
  ...
  if (debug) cat("map_sp_hex: collected", nrow(hex_sp_collected), "hex records before join\n")
  ...
  if (debug) {
    cat("map_sp_hex: created", length(hex_sp), "hex layers\n")
    for (i in seq_along(hex_sp)) {
      cat("  Resolution", res_range[i], ":", nrow(hex_sp[[i]]), "hexagons\n")
    }
  }
...
#'
#' @param sp_ts data.frame from \code{\link{build_ts_sp}} with columns: \code{time}, \code{name}, \code{avg}, \code{std}, \code{lwr}, \code{upr}
#' @param env_ts data.frame from \code{\link{build_ts_env}} with columns: \code{time}, \code{avg}, \code{std}, \code{lwr}, \code{upr}
...
#' }
#'
#' @seealso \code{\link{build_ts_sp}} for species time series data
#' @seealso \code{\link{build_ts_env}} for environmental time series data
#'
...
#' df_sp <- get_sp("Anchovy (Engraulis mordax)", qtr = 1:4, date_range = c("2000-01-01", "2020-12-31"))
#' df_env <- get_env("t_deg_c", qtr = 1:4, date_range = c("2000-01-01", "2020-12-31"), min_depth = 0, max_depth = 100)
#' df_splot <- prep_scatter(df_sp, df_env, env_stat = "mean")
#' }
#'
...
#'
#' @export
prep_scatter <- function(df_sp, df_env, env_stat, max_hours_diff = 72, max_meters_diff = 1000) {
