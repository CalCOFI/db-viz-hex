...
# url_cc         <- "https://file.calcofi.io/data/calcofi.duckdb"
tmp_db         <- here("data/tmp.duckdb")
# cc_db          <- glue("/share/data/swfsc.noaa.gov/calcofi.duckdb")
cc_db          <- here("data/calcofi.duckdb")
...
is_server     <- Sys.info()[["sysname"]]  == "Linux" # (!is_server): Ben's laptop

con_tmp <- dbConnect(duckdb(), dbdir = tmp_db)
res <- dbExecute(con_tmp, glue("ATTACH IF NOT EXISTS '{cc_db}' AS calcofi; USE calcofi"))
...
d |> 
  ...
  select(species_id, common_name, scientific_name, worms_id_old = worms_old_url, itis_id_old = itis_old_url) |> 
  showtable( 
...
d_worms_invalid <- d |> # 1,148 × 8
  ...
  arrange(species_id, worms_invalid_scientific_name)
# View(d_invalid)
# d_invalid |>
#   glimpse()
...
#| tbl-cap: "CalCOFI taxonomic parentage example for anchovies _Clupeiformes_ (species_id: 9)."
#| eval: !expr (!is_server)
...
d_tp_ex <- d_species |> 
  filter(common_name == "anchovies") |>  # species_id: 9; scientific_name: Clupeiformes
  pull(worms_id) |>
...
Upload to the server via [rstudio.calcofi.io](https://rstudio.calcofi.io), **Upload**:

- from: local file on laptop\
  `r here("data/calcofi.duckdb")`

- to: remote file on server\
  `/share/public/data/calcofi.duckdb`

## Get taxonomic children

```{r tbl-taxon_children_example}
#| tbl-cap: "CalCOFI taxonomic children example for rockfishes _Sebastes_ (species_id: 683; worms_id: 126175)."

# TODO: ensure valid taxonID (ie == acceptedNameUsageID)
#  like in get_taxon_parentage()
# TODO: update to USING KEY in Recursive CTEs – DuckDB version 1.3+
...
d_tc <- get_taxon_children(126175, con = con_cc)
